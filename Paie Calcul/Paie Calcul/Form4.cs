...
namespace Paie_Calcul
{
    public partial class Form4 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand(" select distinct * from Paie_initial where Id_Table = '" + textBox1.Text + "' ", cn);
            cn.Open();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("insert into Paie_total (Id_Table,Matricule,FullName,The_Date,Entree_01,Sortie_01,Entree_02,Sortie_02,Entree_03,Sortie_03,Original_Time_Minutes,Total,Temps_Sortie) values(@Id_Table,@Matricule,@FullName,@The_Date,@Entree_01,@Sortie_01,@Entree_02,@Sortie_02,@Entree_03,@Sortie_03,@Original_Time_Minutes,@Total,@Temps_Sortie) ", cn);
                cmd.Parameters.AddWithValue("@Id_Table", textBox1.Text);
        ...
        void Less_time()
        {
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                if (C10 <= (C6 + Ts11)) { dataGridView1.Rows[i].Cells[11].Value = Ts0; }
                if (C10 <= (C4 + Ts11)) { dataGridView1.Rows[i].Cells[11].Value = Ts0; }
                if (C10 <= (C2 + Ts11)) { dataGridView1.Rows[i].Cells[11].Value = Ts0; }
        ...
        /*private void txt1_form4(object sender, EventArgs e)
        {
            if (textBox1.Text != "") { button1.Enabled = true; }
            else { button1.Enabled = false; }
            ...
            cn.Open();
            SqlCommand cmd = new SqlCommand("select Id_Table from Paie_initial", cn);
            SqlDataReader dr = cmd.ExecuteReader();
